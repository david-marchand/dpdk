...
test_no_device_probe_flag(void)
{
	...
	const char *wlinval[][5] = {
		{prgname, prefix, mp_flag, "--no-device-probe", "-a pci:0000:00:00.0" },
		{prgname, prefix, mp_flag, "--no-device-probe", "-b pci:0000:00:00.0" },
	};
	...
	for (unsigned int i = 0; i < RTE_DIM(wlinval); i++) {
		if (launch_proc(wlinval[i]) == 0) {
			printf("Error (line %d) - process did run ok with invalid device probe parameter\n",
				__LINE__);
			return -1;
		}
	}

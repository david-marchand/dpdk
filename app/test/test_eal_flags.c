...
#define no_initial_probing "--no-initial-probing"
#define allow "--allow"
#define vdev "--vdev"
...
test_allow_flag(void)
{
	...
	const char *wlval5[] = {prgname, prefix, mp_flag, eal_debug_logs,
			no_initial_probing, allow, "00FF:09:0B.3"};
	...
	if (launch_proc(wlval4) != 0 ) {
		printf("Error (line %d) - process did not run ok with no-initial-probing\n",
			__LINE__);
		return -1;
	}
	if (launch_proc(wlval5) != 0 ) {
		printf("Error (line %d) - process did not run ok with no-initial-probing + allow\n",
			__LINE__);
		return -1;
	}
...
test_invalid_b_flag(void)
{
	...
	const char *blinval_probing[] =
		{prgname, prefix, mp_flag, eal_debug_logs, "-b", "0:0.0", no_initial_probing};
	...
	if (launch_proc(blinval_probing) == 0) {
		printf("Error (line %d) - process did run ok with blocklist and no-initial-probing\n",
			__LINE__);
		return -1;
	}
...
test_invalid_vdev_flag(void)
{
	...
	const char *vdevval4[] = {prgname, prefix, no_huge, eal_debug_logs,
				bus_debug_logs, no_initial_probing, vdev, "net_ring0"};
	...
	if (launch_proc(vdevval4) != 0) {
		printf("Error (line %d) - process did not run ok with valid vdev value and no-initial-probing\n",
			__LINE__);
		return -1;
	}
	return 0;

...
we wrap the function with ``RTE_VERSION_FUNCTION``, passing the current ABI version,
...
function name and implement it appropriately, then wrap it in a call to ``RTE_DEFAULT_FUNCTION``.

.. code-block:: c

   RTE_DEFAULT_FUNCTION(22,
   struct rte_acl_ctx *,
   rte_acl_create, (const struct rte_acl_param *param, int debug);
   {
...
.. code-block:: c
   ...
   RTE_VERSION_EXPERIMENTAL_FUNCTION(
   struct rte_acl_ctx *
   rte_acl_create, (const struct rte_acl_param *param)
   {

...
"""Check symbol consistency between exports and flags in DPDK libraries."""
...
class VersionMapParser:
    ...
    def __init__(self, mapfile: Path):
    ...
    def _parse_map_file(self):
        ...
        try:
            ...
            print(f"Error reading map file {self.mapfile}: {e}", file=sys.stderr)
            sys.exit(1)
...
class ELFSymbolAnalyzer:
    """Analyze archive files using pyelftools to extract symbol information from contained objects."""
...
class SymbolConsistencyChecker:
    ...
    def __init__(self, mapfile: Path, archive_file: Path):
        self.map_parser = VersionMapParser(mapfile)
    ...
    def check_experimental_consistency(self):
        ...
        for symbol in self.elf_analyzer.experimental_symbols:
            if not self.map_parser.symbol_in_section(symbol, "EXPERIMENTAL"):
                self.errors.append(ValidationError(
                    symbol=symbol,
                    error_type="missing_experimental_export",
    ...
    def check_internal_consistency(self):
        ...
        exported_internal = self.map_parser.get_symbols_by_section("INTERNAL")
        
        # Check 3: Symbols exported as internal but not flagged with __rte_internal
        for symbol in exported_internal:
            if (self.elf_analyzer.symbol_in_text(symbol) and 
                not self.elf_analyzer.symbol_in_internal_section(symbol)):
                self.errors.append(ValidationError(
                    symbol=symbol,
                    error_type="missing_internal_flag",
        ...
        for symbol in self.elf_analyzer.internal_symbols:
            if not self.map_parser.symbol_in_section(symbol, "INTERNAL"):
                self.errors.append(ValidationError(
                    symbol=symbol,
                    error_type="missing_internal_export",
...
def main() -> int:
    ...
    if not args.mapfile.exists():
        print(f"Error: Map file {args.mapfile} does not exist", file=sys.stderr)
        return 1
    ...
    checker = SymbolConsistencyChecker(args.mapfile, args.archive_file)

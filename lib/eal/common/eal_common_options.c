...
eal_reset_internal_config(struct internal_config *internal_cfg)
{
	...
	internal_cfg->no_device_probe = NULL;
...
eal_parse_args(void)
{
	...
	if (args.no_device_probe != NULL) {
		if (args.no_device_probe == (void *)1)
			int_cfg->no_device_probe = strdup("all");
		else
			int_cfg->no_device_probe = strdup(args.no_device_probe);
		if (int_cfg->no_device_probe == NULL) {
			EAL_LOG(ERR, "failed to allocate memory for no device probe parameter");
			return -1;
		}
	}
...
eal_cleanup_config(struct internal_config *internal_cfg)
{
	...
	free(internal_cfg->no_device_probe);

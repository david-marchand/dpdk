...
parse_arg_corelist(const struct rte_argparse_arg *arg, const char *value)
{
	...
	while (*last != '\0') {
		...
		if (!isdigit(*value)) {
			ARGPARSE_LOG(ERR, "argument %s expect digits!", arg->name_long);
			return -EINVAL;
		}
		...
		if (*end == '-') {
			if (min != -1) { /* can't have '-' within a range */
				ARGPARSE_LOG(ERR, "argument %s contains an invalid - character!",
					arg->name_long);
				return -EINVAL;
			}
			...
			ARGPARSE_LOG(ERR, "argument %s has an invalid value!",
				arg->name_long);
			return -EINVAL;
		}
		value = last + 1;

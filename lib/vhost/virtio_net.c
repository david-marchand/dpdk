...
static bool
init_dma_context(struct async_dma_context *dma_ctx, int16_t dma_id, uint16_t vchan_id)
{
	bool is_valid = dma_id >= 0 &&
	...
	if (is_valid) {
		dma_ctx->dma_info = &dma_copy_track[dma_id].vchans[vchan_id];
		dma_ctx->dma_id = dma_id;
		dma_ctx->vchan_id = vchan_id;
	}
	return is_valid;
}
...
vhost_async_dma_transfer_one(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct async_dma_context *dma_ctx, uint16_t flag_idx,
		struct vhost_iov_iter *pkt)
	__rte_requires_shared_capability(&vq->access_lock)
{
	struct async_dma_vchan_info *dma_info = dma_ctx->dma_info;
	...
	if (rte_dma_burst_capacity(dma_ctx->dma_id, dma_ctx->vchan_id) < nr_segs)
		return -1;

	for (i = 0; i < nr_segs; i++) {
		copy_idx = rte_dma_copy(dma_ctx->dma_id, dma_ctx->vchan_id,
				(rte_iova_t)iov[i].src_addr, (rte_iova_t)iov[i].dst_addr,
		...
		if (unlikely(copy_idx < 0)) {
			if (!vhost_async_dma_copy_log) {
				VHOST_DATA_LOG(dev->ifname, ERR,
					...
					dma_ctx->dma_id, dma_ctx->vchan_id);
...
vhost_async_dma_transfer(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct async_dma_context *dma_ctx, uint16_t head_idx,
	...
	__rte_requires_shared_capability(&vq->access_lock)
{
	...
	for (pkt_idx = 0; pkt_idx < nr_pkts; pkt_idx++) {
		ret = vhost_async_dma_transfer_one(dev, vq, dma_ctx, head_idx, &pkts[pkt_idx]);
		if (unlikely(ret < 0))
			break;
	...
	if (likely(nr_copies > 0))
		rte_dma_submit(dma_ctx->dma_id, dma_ctx->vchan_id);
...
static __rte_always_inline uint16_t
vhost_async_dma_check_completed(struct virtio_net *dev, struct async_dma_context *dma_ctx,
		uint16_t max_pkts)
{
	struct async_dma_vchan_info *dma_info = dma_ctx->dma_info;
	uint16_t ring_mask = dma_info->ring_mask;
	 ...
	 */
	nr_copies = rte_dma_completed(dma_ctx->dma_id, dma_ctx->vchan_id, max_pkts, &last_idx,
			&has_error);
...
static __rte_always_inline int
virtio_dev_rx_packed_batch_check(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf **pkts, uint64_t *desc_addrs, uint64_t *desc_iova_addrs,
		uint64_t *lens)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	bool wrap_counter = vq->avail_wrap_counter;
	...
	vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE)
		desc_iova_addrs[i] =  descs[avail_idx + i].addr;

	vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE)
		desc_addrs[i] = vhost_iova_to_vva(dev, vq, desc_iova_addrs[i], &lens[i],
				VHOST_ACCESS_RW);
...
virtio_dev_rx_sync_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf **pkts, uint32_t count, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	return virtio_dev_rx_split(dev, vq, pkts, count, dma_ctx);
}

static __rte_noinline uint32_t
virtio_dev_rx_async_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf **pkts, uint32_t count, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	async_iter_reset(vq->async);
	return virtio_dev_rx_split(dev, vq, pkts, count, dma_ctx);
}
...
vhost_enqueue_single_packed(struct virtio_net *dev, struct vhost_virtqueue *vq,
		...
		uint16_t *nr_buffers, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	uint16_t nr_vec = 0;
	...
	*nr_buffers = 0;
	...
	if (unlikely(mbuf_to_desc(dev, vq, pkt, buf_vec, nr_vec, *nr_buffers, dma_ctx != NULL)
			 < 0))
		return -1;
...
virtio_dev_rx_packed_single(struct virtio_net *dev, struct vhost_virtqueue *vq,
	...
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	...
	if (unlikely(vhost_enqueue_single_packed(dev, vq, pkt, buf_vec, nr_descs, nr_buffers,
			dma_ctx) < 0)) {
...
static __rte_always_inline void
virtio_dev_rx_packed_batch_copy(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf **pkts, uint64_t *desc_addrs, uint64_t *desc_iova_addrs,
		uint64_t *lens, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	uint32_t buf_offset = sizeof(struct virtio_net_hdr_mrg_rxbuf);
	...
	if (dma_ctx == NULL) {
		...
		vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
			host_iova[i] = (void *)(uintptr_t)gpa_to_first_hpa(dev,
					desc_iova_addrs[i] + buf_offset, lens[i], &mapped_len[i]);
		}

		vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
			async_iter_initialize(dev, async);
			async_iter_add_iovec(dev, async,
					(void *)(uintptr_t)rte_pktmbuf_iova_offset(pkts[i], 0),
					host_iova[i], mapped_len[i]);
			async->iter_idx++;
		}
	}
...
static __rte_always_inline int
virtio_dev_rx_packed_batch(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf **pkts, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	uint64_t desc_addrs[PACKED_BATCH_SIZE];
	uint64_t desc_iova_addrs[PACKED_BATCH_SIZE];
	uint64_t lens[PACKED_BATCH_SIZE];

	if (virtio_dev_rx_packed_batch_check(dev, vq, pkts, desc_addrs, desc_iova_addrs, lens)
			== -1)
		return -1;
	...
	if (dma_ctx == NULL) {
		...
		if (rte_dma_burst_capacity(dma_ctx->dma_id, dma_ctx->vchan_id) < PACKED_BATCH_SIZE)
			return -1;
	}

	virtio_dev_rx_packed_batch_copy(dev, vq, pkts, desc_addrs, desc_iova_addrs, lens, dma_ctx);
...
static __rte_always_inline uint32_t
virtio_dev_rx_packed(struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
	...
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	...
	do {
		...
		if (count - pkt_idx >= PACKED_BATCH_SIZE) {
			if (!virtio_dev_rx_packed_batch(dev, vq, &pkts[pkt_idx], dma_ctx)) {
				for (i = 0; i < PACKED_BATCH_SIZE; i++) {
					if (dma_ctx != NULL) {
						slot_idx = (async->pkts_idx + pkt_idx)
								& (vq->size - 1);
		...
		if (virtio_dev_rx_packed_single(dev, vq, pkts[pkt_idx], &num_buffers, &num_descs,
				dma_ctx))
			break;
...
static __rte_noinline uint32_t
virtio_dev_rx_sync_packed(struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
		struct rte_mbuf **__rte_restrict pkts, uint32_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	return virtio_dev_rx_packed(dev, vq, pkts, count, dma_ctx);
}

static __rte_noinline uint32_t
virtio_dev_rx_async_packed(struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
		struct rte_mbuf **__rte_restrict pkts, uint32_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
	...
	__rte_requires_shared_capability(&vq->access_lock)
{
	...
	vhost_async_dma_check_completed(dev, dma_ctx, VHOST_DMA_MAX_COPY_COMPLETE);
		...
		uint16_t vchan_id)
{
	...
	if ((queue_id & 1) == 0)
		n_pkts_cpl = vhost_poll_enqueue_completed(dev, vq, pkts, count, &dma_ctx);
	else
		n_pkts_cpl = async_poll_dequeue_completed(dev, vq, pkts, count, &dma_ctx,
		...
		struct rte_mbuf **pkts, uint32_t count, struct async_dma_context *dma_ctx)
{
	...
	if (dma_ctx == NULL) {
		if (vq_is_packed(dev))
			nb_tx = virtio_dev_rx_sync_packed(dev, vq, pkts, count, dma_ctx);
		else
			nb_tx = virtio_dev_rx_sync_split(dev, vq, pkts, count, dma_ctx);

		vhost_queue_stats_update(dev, vq, pkts, nb_tx);
	} else {
		if (vq_is_packed(dev))
			nb_tx = virtio_dev_rx_async_packed(dev, vq, pkts, count, dma_ctx);
		else
			nb_tx = virtio_dev_rx_async_split(dev, vq, pkts, count, dma_ctx);
		...
		struct rte_mbuf **__rte_restrict pkts, uint16_t count)
{
	...
	return virtio_dev_rx(dev, dev->virtqueue[queue_id], pkts, count, NULL);
}
		...
		uint16_t vchan_id)
{
	...
	return virtio_dev_rx(dev, dev->virtqueue[queue_id], pkts, count, &dma_ctx);
}
...
static __rte_always_inline struct virtio_net_hdr *
desc_extract_hdr(struct virtio_net *dev, struct buf_vector *buf_vec, struct virtio_net_hdr *hdr)
{
	...
	return (struct virtio_net_hdr *)((uintptr_t)buf_vec[0].buf_addr);
}
	...
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	...
	has_hdr = virtio_net_with_host_offload(dev);
...
static __rte_always_inline int
virtio_dev_tx_packed_batch_check(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf **pkts, uintptr_t *desc_addrs, uintptr_t *desc_iova_addrs,
...
virtio_dev_tx_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
		...
		struct async_dma_context *dma_ctx, bool legacy_ol_flags)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	...
	if (dma_ctx == NULL)
		...
		pkts_alloc = pkts_prealloc;

	if (rte_pktmbuf_alloc_bulk(mbuf_pool, pkts_alloc, count)) {
		vq->stats.mbuf_alloc_failed += count;
		goto out;
	}

	for (pkt_idx = 0; pkt_idx < count; pkt_idx++) {
		struct rte_mbuf *pkt = pkts_alloc[pkt_idx];
		struct buf_vector buf_vec[BUF_VECTOR_MAX];
		...
		if (unlikely(fill_vec_buf_split(dev, vq, vq->last_avail_idx + pkt_idx,
				&nr_vec, buf_vec, &head_idx, &buf_len, VHOST_ACCESS_RO) < 0))
			break;
		...
		if (unlikely(virtio_dev_pktmbuf_prep(dev, pkt, buf_len))) {
			/*
		...
		if (unlikely(desc_to_mbuf(dev, vq, buf_vec, nr_vec, pkt, mbuf_pool,
				dma_ctx != NULL))) {
			if (!allocerr_warned) {
				VHOST_DATA_LOG(dev->ifname, ERR, "%s: failed to %s.",
					__func__, dma_ctx == NULL ?
						"copy desc to mbuf" :
						"offload copies to async channel");
				allocerr_warned = true;
			}
			break;
		}
		...
		if (dma_ctx == NULL) {
			if (hdr)
				vhost_dequeue_offload(dev, hdr, pkt, legacy_ol_flags);
		} else {
			slot_idx = (async->pkts_idx + pkt_idx) & (vq->size - 1);
			if (hdr)
	...
	if (dma_ctx == NULL) {
		...
		if (unlikely(pkt_idx != n_xfer)) {
			 ...
			 * for error pkts.
			 */
			async->desc_idx_split -= pkt_idx - n_xfer;
			pkt_idx = n_xfer;
		}

		vq->last_avail_idx += pkt_idx;
		vhost_virtqueue_reconnect_log_split(vq);
	...
	if (unlikely(pkt_idx != count))
		rte_pktmbuf_free_bulk(&pkts_alloc[pkt_idx], count - pkt_idx);

out:
	if (dma_ctx != NULL) {
		/* DMA device may serve other queues, unconditionally check completed. */
		nr_done_pkts = async_poll_dequeue_completed(dev, vq, pkts, pkts_size, dma_ctx,
				legacy_ol_flags);
	}
...
__rte_noinline
static uint16_t
virtio_dev_tx_sync_split_legacy(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mempool *pool, struct rte_mbuf **pkts, uint16_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	return virtio_dev_tx_split(dev, vq, pool, pkts, count, dma_ctx, true);
}

__rte_noinline
static uint16_t
virtio_dev_tx_sync_split_compliant(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mempool *pool, struct rte_mbuf **pkts, uint16_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	return virtio_dev_tx_split(dev, vq, pool, pkts, count, dma_ctx, false);
}

__rte_always_inline
static uint16_t
virtio_dev_tx_sync_split(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mempool *pool, struct rte_mbuf **pkts, uint16_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	if (dev->flags & VIRTIO_DEV_LEGACY_OL_FLAGS)
		return virtio_dev_tx_sync_split_legacy(dev, vq, pool, pkts, count, dma_ctx);
	else
		return virtio_dev_tx_sync_split_compliant(dev, vq, pool, pkts, count, dma_ctx);
}

__rte_noinline
static uint16_t
virtio_dev_tx_async_split_legacy(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mempool *pool, struct rte_mbuf **pkts, uint16_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
...
__rte_noinline
static uint16_t
virtio_dev_tx_async_split_compliant(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mempool *pool, struct rte_mbuf **pkts, uint16_t count,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
	...
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	async_iter_reset(vq->async);
	if (dev->flags & VIRTIO_DEV_LEGACY_OL_FLAGS)
		return virtio_dev_tx_async_split_legacy(dev, vq, pool, pkts, count, dma_ctx);
	else
		return virtio_dev_tx_async_split_compliant(dev, vq, pool, pkts, count, dma_ctx);
}
...
virtio_dev_tx_single_packed(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mempool *mbuf_pool, struct rte_mbuf *pkts[], bool legacy_ol_flags,
		uint16_t pkt_idx, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	static bool allocerr_warned;
	...
	hdr = desc_extract_hdr(dev, buf_vec, &tmp_hdr);
	if (unlikely(desc_to_mbuf(dev, vq, buf_vec, nr_vec, pkts[pkt_idx], mbuf_pool,
			dma_ctx != NULL))) {
		if (!allocerr_warned) {
			VHOST_DATA_LOG(dev->ifname, ERR, "%s: failed to %s.",
				__func__, dma_ctx == NULL ?
	...
	if (dma_ctx == NULL) {
		if (hdr)
			vhost_dequeue_offload(dev, hdr, pkts[pkt_idx], legacy_ol_flags);
	} else {
		slot_idx = (async->pkts_idx + pkt_idx) & (vq->size - 1);
		if (hdr)
			async->pkts_info[slot_idx].nethdr = *hdr;
...
static __rte_always_inline int
virtio_dev_tx_packed_batch(struct virtio_net *dev, struct vhost_virtqueue *vq,
		struct rte_mbuf *pkts[], uint16_t pkt_idx, bool legacy_ol_flags,
		struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	uint32_t buf_offset = sizeof(struct virtio_net_hdr_mrg_rxbuf);
	uintptr_t desc_iova_addrs[PACKED_BATCH_SIZE];
	...
	if (virtio_dev_tx_packed_batch_check(dev, vq, pkts, desc_addrs, desc_iova_addrs, lens,
			ids))
		return -1;
	...
	if (dma_ctx == NULL) {
		...
		vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
			host_iova[i] = (void *)(uintptr_t)gpa_to_first_hpa(dev,
					desc_iova_addrs[i] + buf_offset, pkts[i]->pkt_len,
					&mapped_len[i]);
		}
	...
	if (virtio_net_with_host_offload(dev)) {
		...
		vhost_for_each_try_unroll(i, 0, PACKED_BATCH_SIZE) {
			hdr = (struct virtio_net_hdr *)(uintptr_t)desc_addrs[i];
			if (dma_ctx == NULL) {
				vhost_dequeue_offload(dev, hdr, pkts[i], legacy_ol_flags);
			} else {
				uint16_t slot_idx = (async->pkts_idx + pkt_idx) & (vq->size - 1);
	...
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	...
	do {
		...
		if (count - pkt_idx >= PACKED_BATCH_SIZE) {
			if (!virtio_dev_tx_packed_batch(dev, vq, pkts_alloc, pkt_idx,
					legacy_ol_flags, dma_ctx)) {
				for (i = 0; i < PACKED_BATCH_SIZE; i++) {
					if (dma_ctx != NULL) {
						slot_idx = (async->pkts_idx + pkt_idx)
								& (vq->size - 1);
		...
		if (unlikely(virtio_dev_tx_single_packed(dev, vq, mbuf_pool, pkts_alloc, pkt_idx,
				legacy_ol_flags, dma_ctx)))
			break;
...
out:
	if (dma_ctx != NULL) {
		nr_done_pkts = async_poll_dequeue_completed(dev, vq, pkts, count, dma_ctx,
				legacy_ol_flags);
	}
...
__rte_noinline
static uint16_t
virtio_dev_tx_sync_packed_legacy(struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
		struct rte_mempool *pool, struct rte_mbuf **__rte_restrict pkts,
		uint32_t count, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	return virtio_dev_tx_packed(dev, vq, pool, pkts, count, dma_ctx, true);
}

__rte_noinline
static uint16_t
virtio_dev_tx_sync_packed_compliant(struct virtio_net *dev,
	...
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	return virtio_dev_tx_packed(dev, vq, pool, pkts, count, dma_ctx, false);
}

__rte_always_inline
static uint16_t
virtio_dev_tx_sync_packed(struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
		struct rte_mempool *pool, struct rte_mbuf **__rte_restrict pkts,
		uint32_t count, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	if (dev->flags & VIRTIO_DEV_LEGACY_OL_FLAGS)
		return virtio_dev_tx_sync_packed_legacy(dev, vq, pool, pkts, count, dma_ctx);
	else
		return virtio_dev_tx_sync_packed_compliant(dev, vq, pool, pkts, count, dma_ctx);
}
...
__rte_always_inline
static uint16_t
virtio_dev_tx_async_packed(struct virtio_net *dev, struct vhost_virtqueue *__rte_restrict vq,
		struct rte_mempool *pool, struct rte_mbuf **__rte_restrict pkts,
		uint32_t count, struct async_dma_context *dma_ctx)
	__rte_requires_shared_capability(&vq->access_lock)
	__rte_requires_shared_capability(&vq->iotlb_lock)
{
	async_iter_reset(vq->async);
	if (dev->flags & VIRTIO_DEV_LEGACY_OL_FLAGS)
		return virtio_dev_tx_async_packed_legacy(dev, vq, pool, pkts, count, dma_ctx);
	else
		return virtio_dev_tx_async_packed_compliant(dev, vq, pool, pkts, count, dma_ctx);
}
		...
		int *nr_inflight, struct async_dma_context *dma_ctx)
{
	...
	if (dma_ctx == NULL) {
		if (vq_is_packed(dev))
			nb_rx += virtio_dev_tx_sync_packed(dev, vq, mbuf_pool,
					pkts + nb_rx, count - nb_rx, dma_ctx);
		else
			nb_rx += virtio_dev_tx_sync_split(dev, vq, mbuf_pool,
					pkts + nb_rx, count - nb_rx, dma_ctx);
	} else {
		if (vq_is_packed(dev))
			nb_rx += virtio_dev_tx_async_packed(dev, vq, mbuf_pool,
					pkts + nb_rx, count - nb_rx, dma_ctx);
		else
			nb_rx += virtio_dev_tx_async_split(dev, vq, mbuf_pool,
					pkts + nb_rx, count - nb_rx, dma_ctx);

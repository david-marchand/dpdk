		...
		if (unlikely(copy_idx < 0)) {
			if (!vhost_async_dma_copy_log) {
				VHOST_LOG_DATA(ERR, "(%s) DMA copy failed for channel %d:%u\n",
					dev->ifname, dma_ctx->dma_id, dma_ctx->vchan_id);
	...
	uint16_t max_pkts)
{
	 ...
	 */
	nr_copies = rte_dma_completed(dma_ctx->dma_id, dma_ctx->vchan_id, max_pkts, &last_idx, &has_error);
	...
	struct async_dma_context *dma_ctx)
{
	...
	for (pkt_idx = 0; pkt_idx < count; pkt_idx++) {
		...
		if (mbuf_to_desc(dev, vq, pkts[pkt_idx], buf_vec, nr_vec, num_buffers, dma_ctx != NULL) < 0) {
			vq->shadow_used_idx -= num_buffers;
			break;
		}
...
static __rte_always_inline uint16_t
vhost_clear_queue(int vid, uint16_t queue_id, struct rte_mbuf **pkts,
	uint16_t count, int16_t dma_id, uint16_t vchan_id, bool must_lock)
{
	...
	if ((queue_id & 1) == 0)
		n_pkts_cpl = vhost_poll_enqueue_completed(dev, queue_id, pkts, count, &dma_ctx);
	else
	...
	struct async_dma_context *dma_ctx)
{
	...
	if (!vq_is_packed(dev))
		nb_tx = virtio_dev_rx_split(dev, vq, queue_id, pkts, count, dma_ctx);
	else if (dma_ctx == NULL)
...
{
	return vhost_clear_queue(vid, queue_id, pkts, count, dma_id, vchan_id, true);
}
...
rte_vhost_clear_queue_thread_unsafe(int vid, uint16_t queue_id,
	...
	uint16_t vchan_id)
{
	return vhost_clear_queue(vid, queue_id, pkts, count, dma_id, vchan_id, false);
}
